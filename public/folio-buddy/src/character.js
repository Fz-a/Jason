...
(function (g) {
  ...
  class GrokCharacter {
    ...
    setShape(name, { silent = false } = {}) {
      ...
      if (silent) {
        // Keep outer SVG scale stable — path morphs via shapeSpring only.
        // Avoids the visible "pop" when SHAPE_ZOOM differs between forms.
      } else {
    ...
    setState(name, { resetEyes = false, soft = false } = {}) {
      ...
      this.celebrateAt = (!soft && name === "celebrate") ? this.stateAt + 140 : -1;
      if (!soft) {
        this.trick = null;
        this.spinTurn = null;
        this.hopAt = -1;
    ...
    _updatePointer(now) {
      ...
      if (src && this.svg.getBoundingClientRect) {
        if (now - this.rectAt > 32) {
          this.rectCache = this.svg.getBoundingClientRect();
          this.rectAt = now;
        }
        const rect = this.rectCache;
        if (rect && rect.width > 0) {
          const mapped = this.gazeTarget ? src : mapPointer(rect, src, 0.85, 28, 18, 1.6);
          this.pointer.tx = clamp((mapped.x - (rect.left + rect.width / 2)) / rect.width, -0.75, 0.75) * 28;
          this.pointer.ty = clamp((mapped.y - (rect.top + rect.height / 2)) / rect.height, -0.75, 0.75) * 18;
        }
      } else {
        this.pointer.tx = 0;
        this.pointer.ty = 0;
    ...
    _tick(now) {
      ...
      if (now >= this.trickAt) {
        if (!this.calm && (V_T.has(this.state) || B_T.has(this.state)) && !this.spinTurn && this.hopAt < 0 && !this.trick) {
          const z = Math.random();
