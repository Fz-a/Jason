...
(function (g) {
  ...
  function paintEyes(opt) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      let Kj = Math.sin(now * 42e-5) * 0.7 + Math.sin(now * 0.001) * 0.25;
      let Ko = Math.sin(now * 58e-5) * 0.4;
      if (pointer) {
        Kj += pointer.x * (1.25 - 0.45 * Vn) + pullX;
        Ko += pointer.y * (1.25 - 0.45 * Vn) + pullY;
      } else {
        Kj += pullX;
        Ko += pullY;
      }
